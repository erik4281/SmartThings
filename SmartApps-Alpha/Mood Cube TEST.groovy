		...
		section("Colors") {
			lights.each {light ->
				if (state.lightCapabilities[light.id] == "color") {
					input "color_${sceneId}_${light.id}", "enum", title: "Set this color?", required: false, multiple:false, options: [
                    "On - Custom Color",
                    "Soft White",
                    "White",
                    "Daylight",
	...
	lights.each {light ->
		...
		if (type != "switch") {
			...
			if (type == "level") {
				...
				log.info "NOW RUNNING COLOR-CHANGE"
                //def hueColor = 70
                //def saturation = 100
                ...
                if (color == "Blue")
                {
                    ...
                    def newValue = [hue: "70", saturation: "100", level: level as Integer ?: 100]
                    light.setColor(newValue)
                    log.debug "new value = $newValue"
                }
                ...
                if (color == "Red")
                {
                    ...
                    def newValue = [hue: "100", saturation: "100", level: level as Integer ?: 100]
                    light.setColor(newValue)
                    log.debug "new value = $newValue"
                }
